...
app.get_https("plt.php?q="+param1+"&nav=1",1,whatdef_at_end);
...
var state = { 'page_id': parameterName};
...
app.get_https("plt.php"+"?q="+param+"&w="+who+"&locateat="+locateat,0,whatdef_at_end,at_end_run);
}
////////////////									 
...
fetch('/upload/image/index.php?format='+allowformat+'&filename='+rnd_nm_pho+photo.name, {credentials: "include",method: "POST", body: formData});
...
app.get_https_2("plt.php"+"?q="+formname+"&locateat="+locateat, names,at_end_run,at_end_run_2);
...
const obj_s = JSON.parse(JSON.stringify(result));
...
data = data.replaceAll("{loop_cnt}", i);	
...
sdata = sdata.replaceAll("img_lee","img");			
...
if(upTo(document.activeElement,'form') == null ){
}else{
...
function filterFunction(whatisinput,whatiselement,shouldforcefill) {
  var input, filter, ul, li, a, i,fakevalue;
...
formData.append("photo", file);
 fetch('/upload/image/uplder.php?format='+allowformat+'&filename='+fille_name, {credentials: "include",method: "POST", body: formData}).then(
